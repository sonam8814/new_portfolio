...
export function setCharTimeline(
  ...
  character?.children.forEach((object) => {
    ...
    if (object.name === "screenlight") {
      ...
      const screenMaterial = meshObject.material as THREE.MeshStandardMaterial;
      screenMaterial.transparent = true;
      screenMaterial.opacity = 0;
      screenMaterial.emissive.set("#C8BFFF");
      gsap.timeline({ repeat: -1, repeatRefresh: true }).to(screenMaterial, {
        emissiveIntensity: () => intensity * 8,
        duration: () => Math.random() * 0.6,
        delay: () => Math.random() * 0.1,
      });
      screenLight = meshObject;
    }
  });
